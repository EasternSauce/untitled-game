...
case class MeleeAttack(params: AbilityParams) extends Ability {
  ...
  override def onActiveStart()(implicit game: CoreGame): MeleeAttack = {
    params.targetId.foreach { targetId =>
      if (
        game.gameState.creatures.contains(targetId) &&
        game.gameState.creatures.contains(params.creatureId)
      ) {
        ...
        if (
          ...
          game.queues.localEventQueue.enqueue(
            MeleeAttackHitsCreatureEvent(targetId, id, params.currentAreaId)
          )
        }
      }
    }

    this.copy(params = this.params.modify(_.state).setTo(AbilityState.Finished))
  }
